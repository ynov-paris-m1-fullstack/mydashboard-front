...
import { GET_SALES } from "@/graphql/queries/sale";

export default async function Home() {

  const fetchSales = async () => {
    try {
      // const response = await fetch('http://localhost:3030/api/sales');
      const response = await fetch("http://127.0.0.1:3030/graphql", {
        method: "POST",
        headers: {
  ...
  const totalSales = sales.reduce((total, current) => total + current.totalRevenue, 0);
  const totalQuantity = sales.reduce((total, current) => total + current.quantitySold, 0);
  ...
  console.log("avzerage", averageSaleAmount);
  ...
  return (
    <div>
      <StatCard label="Total sales" value={totalSales} icon={salesIcon} />
      <StatCard label="Quantities sold" value={totalQuantity} icon={salesIcon} />
      <StatCard label="Total order" value={totalOrder} icon={salesIcon} />
      <StatCard label="Average sales" value={averageSaleAmount} icon={salesIcon} />
    </div>
  );
}
